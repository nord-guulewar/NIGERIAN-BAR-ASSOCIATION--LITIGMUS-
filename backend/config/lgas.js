// Local Government Areas (LGAs) for all Nigerian States

const nigerianLGAs = {
  AB: [ // Abia State - 17 LGAs
  ...
  OY: [ // Oyo State - 33 LGAs
    ...
    'Ibarapa North', 'Ido', 'Irepo', 'Iseyin', 'Itesiwaju', 'Iwajowa', 'Kajola', 'Lagelu', 'Ogbomosho North',
    'Ogbomosho South', 'Ogo Oluwa', 'Olorunsogo', 'Oluyole', 'Ona Ara', 'Orelope', 'Ori Ire', 'Oyo East',
    'Oyo West', 'Saki East', 'Saki West', 'Surulere'
  ],
  PL: [ // Plateau State - 17 LGAs
...
const getLGAsByState = (stateCode) => {
  return nigerianLGAs[stateCode] || [];
};

// Get all LGAs with state information
const getAllLGAsWithStates = () => {
  const result = [];
  for (const [stateCode, lgas] of Object.entries(nigerianLGAs)) {
    lgas.forEach(lga => {
      result.push({
        lga,
        stateCode,
        fullName: `${lga}, ${stateCode}`
      });
    });
  }
  return result;
...
// Count total LGAs
const getTotalLGACount = () => {
  return Object.values(nigerianLGAs).reduce((total, lgas) => total + lgas.length, 0);
};

module.exports = {
  nigerianLGAs,
  getLGAsByState,
  getAllLGAsWithStates,
  getTotalLGACount
};
